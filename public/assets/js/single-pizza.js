...
// Logic to retrieve the pizzaId data from the URL and make a GET request for the associated pizza
function getPizza() {
  // get ID of pizza
  const searchParams = new URLSearchParams(document.location.search.substring(1));
  const pizzaId = searchParams.get('id');

  // GET request pizzaInfo
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .catch(err => {
      ...
      window.history.back(); // takes you back to the homepage
    });
}
